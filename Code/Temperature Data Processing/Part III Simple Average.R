...
NUTS_3P_23$avg_tmp <- exact_extract(r23, NUTS_3P, 'mean')
...
for (i in 1:length(dates23)){
  tryCatch({
    ...
    #Summarize 
    nuts_a<-NUTS_3P
    nuts_a$avg_tmp<-exact_extract(r, NUTS_3P,'mean')
...
names(NUTS_3b)<-c('NUTSID','LEVL','CNTR','NAME_LATIN','NAME','MOUNT','URBN','COAST','FID','geo','avg_tmp_a','Date')

#try formatting temp as numeric 
NUTS_3b$avg_tmp<-as.numeric(NUTS_3b$avg_tmp_a)

#Remove Geo Column and format as data frame
NUTS_3b_df<-as.data.frame(NUTS_3b)

#Remove Geo
NUTS_3b_dfa<-subset(NUTS_3b_df, select=c(NUTSID,LEVL,CNTR,NAME_LATIN,NAME,MOUNT,URBN,COAST,FID,avg_tmp,Date))

#Export to csv 
write.csv(NUTS_3b_dfa, file = '/Users/amandanorton/Desktop/temp_avg_2023.csv')
remove(NUTS_3b)
